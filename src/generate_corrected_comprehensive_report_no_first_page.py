#!/usr/bin/env python3
"""
Corrected Comprehensive Model Comparison Report Generator (No First Page)

This script generates a refined report using corrected metrics, skipping the first page
and creating better formatting for the remaining content.
...
def create_improved_metrics_tables(data, pdf):
    """Create improved metrics tables with better spacing and layout."""
    print("📊 Creating improved metrics tables...")
    
    if not data:
        return
    
    # Basic Statistics Table - Improved layout
    ...
    if basic_stats:
        ...
        # Style table with better spacing
    ...
    if dist_tests:
        ...
        ax.set_title('Corrected Distribution Test Results\n(Standardized MMD computation)', 
                    fontsize=16, fontweight='bold', pad=30, y=0.98)
        
        pdf.savefig(fig, dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
    
    # Tail Risk Metrics Table - Improved layout
    ...
    if tail_metrics:
        ...
        ax.set_title('Corrected Tail Risk Metrics\n(Proper sign conventions applied)', 
                    fontsize=16, fontweight='bold', pad=30, y=0.98)
        
        pdf.savefig(fig, dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
    
    # Volatility Metrics Table - Improved layout
    ...
    if vol_metrics:
        ...
        ax.set_title('Corrected Volatility Dynamics Metrics\n(Non-negative constraints applied)', 
                    fontsize=16, fontweight='bold', pad=30, y=0.98)
...
def create_robust_metrics_table(data, pdf):
    ...
    ax.set_title('Robust Metrics with Bootstrap Statistics\n(5 runs, 95% confidence intervals)', 
                fontsize=16, fontweight='bold', pad=30, y=0.98)
...
def create_corrected_performance_ranking(data, pdf):
    ...
    ax.set_title('Corrected Model Performance Ranking\n(Lower Score = Better Performance)', 
                fontsize=16, fontweight='bold', pad=30, y=0.98)
...
def main():
    """Main function to generate corrected comprehensive report without first page."""
    print("🚀 Generating Corrected Comprehensive Model Comparison Report (No First Page)...")
    ...
    # Create PDF
    output_path = "results/comprehensive_model_comparison_report_corrected_fixed.pdf"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    
    with PdfPages(output_path) as pdf:
        print(f"📄 Creating corrected PDF (no first page): {output_path}")
        
        # 1. Improved Metrics Tables (now first page)
        create_improved_metrics_tables(corrected_data, pdf)
        
        # 2. Robust Metrics Table
        ...
        # 4. Corrected Performance Ranking
    ...
    print(f"✅ Corrected comprehensive report (no first page) generated: {output_path}")
    print(f"🔧 First page formatting issues have been resolved")
